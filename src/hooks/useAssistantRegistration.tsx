...
interface UseAssistantRegistrationProps {
  gymCode: string | undefined;
}

export const useAssistantRegistration = ({ gymCode }: UseAssistantRegistrationProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verify that the code is for assistant role
      const { data: roleData, error: roleError } = await supabase.rpc('get_role_from_code', {
        registration_code: gymCode
      });

      if (roleError || roleData !== 'assistant') {
        toast.error("Questo codice non è valido per la registrazione come assistente");
        setIsLoading(false);
        return;
      }
